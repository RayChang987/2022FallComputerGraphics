...
int L2(int r1, int r2, int b1, int b2, int g1, int g2) {
    return sqrt((r2 - r1) * (r2 - r1) + (b2 - b1) * (b2 - b1) + (g2 - g1) * (g2 - g1));
}
bool TargaImage::Quant_Populosity()
{
    data_to_imgRGBA();
    const int mxN = 4e4;
    vector<pair<int, int>> feq(mxN, { 0, 0 });
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            feq[encoded_data].first++;
        }
    }   
    for (int i = 0; i < mxN; ++i) {feq[i].second = i;}
    sort(feq.begin(), feq.end(), [](pair<int, int>& a, pair<int, int>& b){
        return a > b;
     });
    int colorN = 256;
    for (int i = 0; i <256; ++i) {
        int r = feq[i].second >> 10<<3;
        feq[i].second -= (r << 10);
        int g = feq[i].second >> 5<<3;
        feq[i].second -= (g<<5);
        int b= feq[i].second<<3;
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            for (int k = 0; k < colorN; ++k) {
                int dis = L2(img_RGBA[0][i][j], colors[k][0], img_RGBA[1][i][j], colors[k][1], img_RGBA[2][i][j], colors[k][2]);
                if (dis < mn) {
                    mnI = k;
                    mn = dis;
                }
            }
            img_RGBA[0][i][j] = colors[mnI][0];
            img_RGBA[1][i][j] = colors[mnI][1];
            img_RGBA[2][i][j] = colors[mnI][2];
        }
    }
    //for (auto& v : colors) {
