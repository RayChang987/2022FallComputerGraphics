...
//=========================================================================
{
	...
	if ( maze ) {
		...
		//}
		//system("PAUSE");
		//glGetFloatv(GL_PROJECTION_MATRIX, t_perspectiveGL);
		//cout << "{" << maze->viewer_fov << " " << aspect << "}" << endl;
		//// 45 1
		//for (int i = 0; i < 4; ++i) {
		//	for (int j = 0; j < 4; ++j) {
		//		cout << t_perspectiveGL[j * 4 + i] << " ";
		//	}
		//	cout << endl;
		//}
		glMatrixMode(GL_MODELVIEW);
		...
		//cout << "{" << viewer_pos[Maze::X] << " " << viewer_pos[Maze::Y] << " " << viewer_pos[Maze::Z] << "}" << endl;
		//cout << "{" << viewer_pos[Maze::X] + sin(Maze::To_Radians(maze->viewer_dir)) << " " << viewer_pos[Maze::Y] << " " << viewer_pos[Maze::Z] + cos(Maze::To_Radians(maze->viewer_dir)) <<"}" << endl;
		//check the difference between GL
		//cerr << "Comparaion Start:" << endl;
		//cerr << "GL: " << endl;;
		/*cout << "GL MODEL" << endl;
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
				cout << t_modelviewGL[j * 4 + i] << " ";
			}
			cout << endl;
		}
		cout << "GL VIEWER" << endl;
